...
### 1.5 设计范围与基线
本文档描述当前可运行的前后端分离单体系统，并以业务场景清单中的 `UC01`～`UC09` 作为功能设计范围。  
各用例的参与者、前置条件、主流程和异常流程由用例清单统一说明；接口明细、测试用例和完整追溯关系分别在接口/追溯材料和测试文档中维护，本文档不重复展开。  
后续微服务改造方案见《微服务划分图》《服务接口清单》《数据表归属方案》，不与当前单体基线混写。
