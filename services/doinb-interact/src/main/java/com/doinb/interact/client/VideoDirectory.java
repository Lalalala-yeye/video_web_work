...
/** 只通过视频服务内部接口取视频信息，不直接访问 videos 表。 */
@Component
public class VideoDirectory {
    ...
    public VideoDTO findById(Integer id) {
        ...
        CustomResponse resp = serviceClient.get(
                ...
                InternalPaths.VIDEOS + "/" + id);
        if (resp.getCode() != 200 || resp.getData() == null) {
            return null;
        }
        return toVideo(resp.getData());
    }

    /** 订阅动态：关注作者的已发布视频（仅 status=1，按时间倒序）。 */
    ...
    private static VideoDTO toVideo(Object value) {
        ...
        video.setDescription(asString(map.get("description")));
        video.setAuthorId(asInteger(map.get("authorId")));
        video.setAuthorNickname(asString(map.get("authorNickname")));
        video.setAuthorAvatar(asString(map.get("authorAvatar")));
        ...
        video.setReportCount(asInteger(map.get("reportCount")));
        return video;
    }
