...
- 《需求追溯矩阵》
- 《软件开发计划》
- 《doinb 视频平台业务场景（用例）清单》
- 《软件工程基础实践》课程任务书
- GB/T 8567-2006 计算机软件文档编制规范

### 1.5 设计范围与基线
本文档描述当前可运行的前后端分离单体系统，并以《需求追溯矩阵》中的 `UC-01`～`UC-15` 作为唯一功能设计基线。概要设计中的用例名称、组件关系、接口归属和测试追溯均使用该编号，不再另设场景编号。
各用例的参与者、前置条件、主流程和异常流程以《软件需求规格说明书（2）》为准；接口与测试编号以《需求追溯矩阵》为准。
后续微服务改造方案在服务边界和数据归属确定后另行设计，不与当前单体基线混写。
...
#### 2.3.1 设备
- **服务端**：云服务器或高性能虚拟机（建议配置：2核CPU、4GB内存以上，并预留 20GB 以上磁盘空间存储媒体资源，与《软件需求规格说明书》3.4.2 一致）。
...
### 2.5 功能需求
根据《需求追溯矩阵》和当前代码实现，系统包含 `UC-01`～`UC-15` 共十五个用例。本节按唯一用例编号说明组件级设计范围；每个用例配一张使用 LaTeX TikZ 建模的 UML 组件级顺序图，图中的组件、接口调用、返回路径和关键分支均与当前前后端代码、数据库或基础设施保持对应。

1. **UC-01 用户注册**：访客提交用户名、密码和确认密码，系统完成格式与用户名唯一性校验，使用 BCrypt 保存密码并创建用户记录。

![UC01 用户注册组件级顺序图](img/UC01用户注册组件级顺序图.png)

*图 2-1 UC-01 用户注册组件级顺序图*

2. **UC-02 用户登录/登出**：用户或管理员提交登录凭据，系统校验身份并签发 JWT；前端在登出时清理当前会话信息，受保护请求统一经过 JWT 过滤器。

![UC02 用户登录登出组件级顺序图](img/UC02用户登录登出组件级顺序图.png)

*图 2-2 UC-02 用户登录/登出组件级顺序图*

3. **UC-03 个人信息维护**：登录用户查询本人资料，修改昵称和简介，上传头像，并在保存后获得更新后的用户信息。

![UC03 个人信息维护组件级顺序图](img/UC03个人信息维护组件级顺序图.png)

*图 2-3 UC-03 个人信息维护组件级顺序图*

4. **UC-04 浏览与播放视频**：访客或登录用户浏览已发布视频列表、进入详情并通过静态媒体地址在线播放；非公开视频按用户身份和所有权控制可见性。

![UC04 浏览与播放视频组件级顺序图](img/UC04浏览与播放视频组件级顺序图.png)

*图 2-4 UC-04 浏览与播放视频组件级顺序图*

5. **UC-05 播放历史**：登录用户播放视频时保存或更新观看进度，并可在个人页面查询历史记录后从上次进度继续播放。

![UC05 播放历史组件级顺序图](img/UC05播放历史组件级顺序图.png)

*图 2-5 UC-05 播放历史组件级顺序图*

6. **UC-06 上传与编辑视频**：作者上传视频和封面、查询本人投稿、修改元数据和可见性；新投稿进入待审核状态，文件与视频记录分别保存到本地目录和数据库。

![UC06 上传与编辑视频组件级顺序图](img/UC06上传与编辑视频组件级顺序图.png)

*图 2-6 UC-06 上传与编辑视频组件级顺序图*

7. **UC-07 举报视频**：登录用户对已发布视频提交举报，系统防止重复举报和自举报并统计有效举报；管理员可查询进入举报复审状态的视频。

![UC07 举报视频组件级顺序图](img/UC07举报视频组件级顺序图.png)

*图 2-7 UC-07 举报视频组件级顺序图*

8. **UC-08 直播管理**：主播创建直播间并执行开播、停播，观众浏览和进入直播间；Spring Boot 维护房间与直播状态，OBS 和 SRS 承担推流及播放分发。直播评论归入 UC-10。

![UC08 直播管理组件级顺序图](img/UC08直播管理组件级顺序图.png)

*图 2-8 UC-08 直播管理组件级顺序图*

9. **UC-09 关注与订阅动态**：登录用户关注或取消关注其他用户，查询关注状态，并在订阅动态中查看关注作者的公开视频和正在进行的直播。

![UC09 关注与订阅动态组件级顺序图](img/UC09关注与订阅动态组件级顺序图.png)

*图 2-9 UC-09 关注与订阅动态组件级顺序图*

10. **UC-10 发表评论**：登录用户在视频或直播间下发表评论并分页查看评论；系统通过 `targetType` 区分视频评论和直播评论，并校验评论目标及内容。

![UC10 发表评论组件级顺序图](img/UC10发表评论组件级顺序图.png)

*图 2-10 UC-10 发表评论组件级顺序图*

11. **UC-11 赞踩互动**：登录用户对视频或评论执行点赞、点踩、取消或切换操作，系统维护本人反应状态和聚合计数，并按规则触发互动通知。

![UC11 赞踩互动组件级顺序图](img/UC11赞踩互动组件级顺序图.png)

*图 2-11 UC-11 赞踩互动组件级顺序图*

12. **UC-12 关键词搜索**：访客或登录用户按关键词检索已发布视频、直播间和用户，系统分别限制各类结果数量并返回统一搜索结果。

![UC12 关键词搜索组件级顺序图](img/UC12关键词搜索组件级顺序图.png)

*图 2-12 UC-12 关键词搜索组件级顺序图*

13. **UC-13 通知**：登录用户查询通知列表和未读数量，将单条或全部通知标记为已读；通知由视频互动、评论互动、私信和审核通过等业务事件产生。

![UC13 通知组件级顺序图](img/UC13通知组件级顺序图.png)

*图 2-13 UC-13 通知组件级顺序图*

14. **UC-14 私信**：登录用户与另一用户打开或复用唯一双人会话，分页查看历史消息并发送新消息，非会话成员不得访问会话内容。

![UC14 私信组件级顺序图](img/UC14私信组件级顺序图.png)

*图 2-14 UC-14 私信组件级顺序图*

15. **UC-15 管理员审核**：管理员查询待审核视频、预览内容并执行通过、驳回或删除；通过后视频公开并通知作者，驳回后视频转为私密。

![UC15 管理员审核组件级顺序图](img/UC15管理员审核组件级顺序图.png)

*图 2-15 UC-15 管理员审核组件级顺序图*
...
#### 3.2.1 用户接口
用户接口主要通过 Web 页面呈现。概要设计按《需求追溯矩阵》的用例编号说明主要输入输出，具体路径、HTTP 方法和测试编号由追溯矩阵维护。

| 关联用例 | 主要操作 | 输入数据 | 输出/响应信息 |
| :--- | :--- | :--- | :--- |
| **UC-01** | 注册账号 | 用户名、密码、昵称 | 注册结果、用户标识 |
| **UC-02** | 用户/管理员登录、查询登录信息、退出 | 用户名、密码、JWT | JWT、当前用户信息或退出结果 |
| **UC-03** | 查询或修改资料、查询公开信息、上传头像 | 用户ID、昵称、简介、头像文件 | 用户资料、头像地址或操作结果 |
| **UC-04** | 查询公开视频列表和视频详情、在线播放 | 视频ID、分页参数 | 视频信息、作者信息和媒体地址 |
| **UC-05** | 保存播放进度、查询播放历史 | 视频ID、播放进度、分页参数 | 历史记录、续播进度或操作结果 |
| **UC-06** | 上传视频、查询投稿、编辑元数据、设置可见性 | 视频和封面文件、标题、描述、视频ID、可见性 | 投稿信息、审核状态或操作结果 |
| **UC-07** | 提交视频举报、查询举报复审列表 | 视频ID、举报原因、分页参数 | 举报结果、举报数量或复审列表 |
| **UC-08** | 查询、创建、开始或结束直播 | 直播间ID、标题 | 直播间信息、推流密钥、播放地址和直播状态 |
| **UC-09** | 关注、取消关注、查询状态、查看订阅动态 | 目标用户ID、分页参数 | 关注状态或订阅内容列表 |
| **UC-10** | 发表评论、查询评论 | 目标ID、目标类型、评论内容、分页参数 | 评论信息或评论列表 |
| **UC-11** | 查询反应汇总、对视频或评论赞踩 | 视频/评论ID、互动类型 | 本人状态、点赞数和点踩数 |
| **UC-12** | 按关键词搜索 | 搜索关键词、各类结果数量参数 | 视频、直播和用户搜索结果 |
| **UC-13** | 查询通知、查询未读数、标记已读 | 通知ID、分页参数 | 通知列表、未读数或已读结果 |
| **UC-14** | 打开会话、查询消息、发送消息 | 对方用户ID、会话ID、消息内容 | 会话信息、消息列表或发送结果 |
| **UC-15** | 查询待审视频、通过、驳回或删除 | 视频ID、分页参数、审核操作 | 待审列表、审核状态或处理结果 |
...
#### 3.3.3 数据结构与程序关系
各用例与主要程序组件、数据结构的关系如下。这里只描述组件级职责，接口操作与测试编号见《需求追溯矩阵》，可测试规则见 `uml/模型测试基线.yaml`。

| 用例 | 主要协作组件 | 主要数据结构 |
| :--- | :--- | :--- |
| **UC-01** | RegisterView、UserAccountController、UserAccountService、UserMapper | User |
| **UC-02** | LoginView、UserAccountController、JWT Filter、UserAccountService、UserMapper | User |
| **UC-03** | ProfileView、UserAccountController、UserController、UserAccountService、UserService、UserMapper、头像文件映射 | User、uploads/avatars |
| **UC-04** | HomeView、VideoDetailView、VideoController、VideoService、VideoMapper、静态资源映射 | Video、uploads |
| **UC-05** | VideoDetailView/ProfileView、VideoController、VideoService、PlayHistoryMapper | PlayHistory、Video |
| **UC-06** | StudioUploadView/StudioEditView、VideoController、VideoService、VideoMapper、文件存储 | Video、uploads |
| **UC-07** | VideoDetailView/AdminReportView、VideoController、AdminVideoController、VideoService、VideoReportMapper | VideoReport、Video |
| **UC-08** | StudioLiveView/LiveListView/LiveRoomView、LiveRoomController、LiveRoomService、LiveStreamHelper、SRS、OBS | LiveRoom |
| **UC-09** | UserShowcaseView/SubscribeView、SubscriptionController、SubscriptionService、相关 Mapper | Subscription、User、Video、LiveRoom |
| **UC-10** | VideoDetailView/LiveRoomView、CommentController、CommentService、相关 Mapper | Comment、Video、LiveRoom |
| **UC-11** | VideoDetailView/CommentItem、ReactionController、ReactionService、互动 Mapper、NotificationService | VideoReaction、CommentReaction、Notification |
| **UC-12** | SearchView、SearchController、SearchService、Video/LiveRoom/User Mapper | Video、LiveRoom、User |
| **UC-13** | TopNavigation、NotificationController、NotificationService、NotificationMapper | Notification、User |
| **UC-14** | MessageRoomView、MessageController、MessageService、DmRoom/DmMessage Mapper、NotificationService | DmRoom、DmMessage、Notification、User |
| **UC-15** | AdminPendingView/AdminVideoPreviewView、AdminVideoController、AdminVideoService、相关 Mapper、NotificationService、文件存储 | Video、Notification、uploads |
